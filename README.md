...
![Skills](https://img.shields.io/badge/skills-16-success)
...
flowchart TD
    ...
    B --> C["zz-pm-prd"]
    B --> D["zz-pm-delivery-pack"]
    B --> E["zz-pm-review-board"]
    B --> F["zz-pm-tracking-spec"]
    B --> G["zz-pm-experiment"]
    B --> H["zz-pm-postmortem"]
    B --> I["zz-pm-prioritization"]
    B --> J["zz-pm-roadmap"]
    B --> K["zz-pm-analytics"]
    B --> L["zz-pm-competitor"]
    B --> M["zz-pm-survey"]
    B --> N["zz-pm-prototype-workflow"]
    N --> O["zz-pm-proto-image"]
    N --> P["zz-pm-proto-url"]
    N --> Q["zz-pm-proto-pencil"]
```

简化理解：

1. 所有产品任务先进入 [`zz-pm-workflow`](./.codex/skills/zz-pm-workflow/SKILL.md)
2. `PRD` 任务切到 [`zz-pm-prd`](./.codex/skills/zz-pm-prd/SKILL.md)
3. 研发交付包任务切到 [`zz-pm-delivery-pack`](./.codex/skills/zz-pm-delivery-pack/SKILL.md)
4. 原型任务切到 [`zz-pm-prototype-workflow`](./.codex/skills/zz-pm-prototype-workflow/SKILL.md)
5. 原型子系统再分流到 `image / url / pencil`
6. 其余任务按能力域切到对应专项技能
...
| [`zz-pm-delivery-pack`](./.codex/skills/zz-pm-delivery-pack/SKILL.md) | Delivery pack generation | generate a dev-ready `proposal/spec/design/tasks` pack from a confirmed PRD and review outcome | Trigger this when PRD review is done and you need a delivery pack that engineering can start from.<br>Trigger this when you do not want engineering to receive only a PRD, but a structured pack of implementation docs.<br>Trigger this when the PRD has been updated after a requirement change and the delivery docs must be synced by impact scope. |
...
| Docs, Delivery & Review | [`zz-pm-prd`](./.codex/skills/zz-pm-prd/SKILL.md), [`zz-pm-delivery-pack`](./.codex/skills/zz-pm-delivery-pack/SKILL.md), [`zz-pm-review-board`](./.codex/skills/zz-pm-review-board/SKILL.md) |
...
- `Delivery pack task` -> [`zz-pm-delivery-pack`](./.codex/skills/zz-pm-delivery-pack/SKILL.md) -> `proposal / spec / design / tasks`
...
| [`zz-pm-delivery-pack/references/`](./.codex/skills/zz-pm-delivery-pack/references/) | Delivery-pack templates for `proposal / spec / design / tasks` |
...
- [Delivery Pack](./.codex/skills/zz-pm-delivery-pack/SKILL.md)
